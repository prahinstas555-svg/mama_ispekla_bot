// Безопасное подключение Telegram (работает и в обычном браузере)
...
function renderProducts() {
  ...
  DATA.products.filter(p => p.category === activeCat).forEach(p => {
    ...
    card.innerHTML = `
      <img src="images/${p.image}" alt="${p.name}" onerror="this.style.background='#eee'">
      <div class="info">
        <h3>${p.name}</h3>
        <div class="desc">${p.description}</div>
        <div class="price">${p.price} ₽ ${p.unit}</div>
        ${qty === 0
          ? `<button onclick="addItem(${p.id})">В корзину</button>`
          : `<div class="qty">
...
function addItem(id) { cart[id] = 1; renderProducts(); updateCartBar(); }
...
document.getElementById('sendOrder').onclick = () => {
  ...
  const items = Object.keys(cart).map(id => {
    const p = DATA.products.find(x => x.id == id);
    return { name: p.name, qty: cart[id], price: p.price, sum: p.price * cart[id] };
  });
